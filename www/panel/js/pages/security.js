...
class SecurityPage extends Page
{
	...
	init()
	{
		...
		Promise.all([
			...
			config.user.level === 'manager' ? Ajax.get('/api/contributor/users', {data: {type: 'contributor'}}) : Promise.resolve({response: []}),
			config.user.level === 'manager' ? Ajax.get('/api/contributor/users', {data: {type: 'manager'}}) : Promise.resolve({response: []}),
		]).then(results =>
		{
			...
			[
				...
				[
					Node.h2([
						...
						[
							config.user.level === 'manager' ? Node.span({className: 'icon', click: () => { self.addRole(); }, dataset: {tooltip: Translator.get('security.role.add')}}, 'add') : null,
				...
				[
					Node.h2([
						...
						Node.div({className: 'small_action'},
						[
							config.user.level === 'manager' ? Node.span({className: 'icon', click: () => { self.addGroup(); }, dataset: {tooltip: Translator.get('security.group.add')}}, 'add') : null,
				...
				[
					Node.h2([
						...
						Node.div({className: 'small_action'},
						[
							config.user.level === 'manager' ? Node.span({className: 'icon', click: () => { self.addConsumer(); }, dataset: {tooltip: Translator.get('security.consumer.add')}}, 'add') : null,
					...
					[
						...
						Node.span(safeHtml(u.name))
					])))
				]),

				config.user.level === 'manager' ? Node.section(
				[
					Node.h2([
	...
	showRole(id)
	{
		...
		Ajax.get('/api/contributor/role/' + encodeURIComponent(id)).then(result =>
		{
			...
			Node.append(div,
			[
				Node.h2(safeHtml(result.response.name)),
				Node.p(Translator.get('security.role.users')),
				Node.ol({classList: 'short'}, result.response.users.map(u => Node.li({className: 'card'}, [Node.span({className: 'icon'}, 'frame_person'), safeHtml(u.name)]))),
	...
	showConsumer(id)
	{
		...
		Ajax.get('/api/contributor/user/' + encodeURIComponent(id)).then(result =>
		{
			...
			[
				...
				Node.ol({classList: 'short'}, result.response.groups.map(g => Node.li({className: 'card'}, [Node.span({className: 'icon'}, 'group'), safeHtml(g.name)]))),
				Node.p([
